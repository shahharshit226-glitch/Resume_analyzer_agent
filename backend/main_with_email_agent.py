...
async def send_report(data: EmailRequest):
    try:
        ...
        if not EMAIL_CONFIG.get("EMAIL_ADDRESS") or not EMAIL_CONFIG.get("EMAIL_PASSWORD"):
            raise HTTPException(status_code=500, detail="Email is not configured in backend .env")
        ...
        <html><body style="font-family: Arial; line-height:1.6;">
            ...
            <p><b>Overall Score:</b> {score}/100</p>
            ...
            <hr/><p style="color:gray;">Generated by AI AgenticHire</p>
        </body></html>
        """
        ...
        try:
            ...
            server.sendmail(EMAIL_CONFIG["EMAIL_ADDRESS"], [data.email], message.as_string())
        finally:
            server.quit()
