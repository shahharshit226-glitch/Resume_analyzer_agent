# """
# Email Agent - Automated Resume Processing System
...
# # Email configuration
# EMAIL_CONFIG = {
#     "IMAP_SERVER": os.getenv("IMAP_SERVER"),  # Gmail IMAP
#     "SMTP_SERVER": os.getenv("SMTP_SERVER"),  # Gmail SMTP
#     "SMTP_PORT": int(os.getenv("SMTP_PORT", 587)), # Changed default to 587 for TLS
#     "EMAIL_ADDRESS": os.getenv("EMAIL_ADDRESS"),  # Replace with your email
#     "EMAIL_PASSWORD": os.getenv("EMAIL_PASSWORD"),     # Replace with Gmail App Password
#     "CHECK_INTERVAL": int(os.getenv("CHECK_INTERVAL", 60)),  # Check every 60 seconds
#     "ATS_THRESHOLD": 70,   # Minimum ATS score to pass
# }
...
#             if filename:
#                 # Check if file is resume format
#                 file_extension = filename.lower().split('.')[-1]
#                 if file_extension in ['pdf', 'docx', 'txt', 'doc']:
...
#     def process_email(self, email_message, email_id: str):
#         """
#         Process a single email with resume attachment
...
#             if not attachments:
#                 self._log(f"⚠️ No resume attachment from {sender_email} — skipping")
#                 self.processed_emails.add(email_id)
#                 return
            
#             self._log(f"📎 Found {len(attachments)} attachment(s) from {sender_email}")
            
#             # Analyze first resume attachment
#             resume_attachment = attachments[0]
#             self._log(f"🔍 Analyzing resume: {resume_attachment['filename']} from {sender_email}")
            
#             # Perform analysis
#             analysis_results = self.analyze_resume_file(resume_attachment)

#             # analysis result logged via shortlisted/rejected below
...
#                 if is_shortlisted:
#                     self._log(f"✅ SHORTLISTED — {sender_email} scored {ats_score} (threshold: {EMAIL_CONFIG['ATS_THRESHOLD']})")
#                     subject = "🎉 Your Resume Has Been Shortlisted!"
#                 else:
#                     self._log(f"❌ REJECTED — {sender_email} scored {ats_score} (threshold: {EMAIL_CONFIG['ATS_THRESHOLD']})")
#                     subject = "Thank You for Your Application"

#                 # Generate response email
#                 html_content = self.generate_response_email(
#                     sender_name,
#                     analysis_results,
#                     is_shortlisted
#                 )
...
#                 # Process email
#                 self.process_email(email_message, email_id.decode())
...
class EmailAgent:
    ...
    def process_email(self, email_message, email_id: str, raw_size: int = 0):
        """
        ...
        try:
            ...
            if not attachments:
                self._log(f"⚠️ No valid resume attachment from {sender_email} — skipping")
                self.processed_emails.add(email_id)
                return
    ...
    def check_emails(self):
        ...
        try:
            ...
            for email_id in email_ids:
                ...
                self.process_email(email_message, email_id.decode(), raw_size)
