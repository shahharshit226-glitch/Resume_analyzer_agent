...
class EmailAgent:
    ...
    def _apply_gmail_label(self, mail_conn, imap_email_id, label_name: str):
        """
        Copy email to the label folder and mark as \\Deleted in INBOX.
        NOTE: We do NOT call expunge() here — expunge renumbers all sequence IDs
        in the session, breaking subsequent fetches in the same loop.
        A single expunge() is called ONCE after the entire batch is processed.
        """
        try:
            self._ensure_gmail_label(mail_conn, label_name)
            eid = imap_email_id if isinstance(imap_email_id, bytes) else imap_email_id.encode()
    ...
    def _route_as_business_email(
        ...
        """
        Called when an email has no resume attachment.
        1. Classifies: Finance / Logistics / Resume / Uncategorized
        2. Saves to categorized_mails (correct column names: from_email, from_name, …)
        3. Applies the matching Gmail label so it lands in the right folder
        """
        subject = email_message.get("Subject", "(No Subject)")
        ...
        # ── 1. Classify ───────────────────────────────────────────────────────
        try:
            from email_classifier import classify_email, get_category_config, log_classification
            category = classify_email(subject, snippet)
            cfg      = get_category_config(category)
            log_classification(sender_email, subject, category)
        except ImportError:
        ...
        # ── 2. Save to DB ─────────────────────────────────────────────────────
        try:
            from candidates_db import save_business_email, init_categorized_mails_db
            init_categorized_mails_db()          # ensures table exists (idempotent)
            save_business_email(
    ...
    def check_emails(self):
        ...
        Key design decisions that prevent the 'NoneType is not subscriptable' crash:

        1. Use UID SEARCH / UID FETCH instead of sequence-number SEARCH / FETCH.
           UIDs are permanent identifiers that never change within a session,
           even after COPY + \\Deleted flags are set on other messages.
           Sequence numbers (the default) get renumbered the moment any message
           in the mailbox is expunged — so after email 1 is labelled and expunged,
           email 2's sequence number no longer exists, and fetch returns None.

        2. Pre-fetch ALL raw bytes into memory before calling process_email().
           This way the IMAP session state is irrelevant during processing —
           we already have everything we need locally.

        3. Expunge ONCE at the very end, after all emails are processed.
           This clears the \\Deleted flags set by _apply_gmail_label() in one shot.
        """
        try:
            mail = self.connect_imap()
            if not mail:
            ...
            # ── Use UID search — UIDs never renumber ──────────────────────────
            ...
            self._log(f"📬 Found {len(uid_list)} new email(s) to process")

            max_per_cycle = SECURITY_CONFIG["MAX_EMAILS_PER_CYCLE"]
            if len(uid_list) > max_per_cycle:
                self._log(
                    f"⚠️ {len(uid_list)} emails in inbox — "
                    f"processing first {max_per_cycle} only (DOS protection)"
                )
                uid_list = uid_list[:max_per_cycle]

            # ── Step 1: Pre-fetch ALL raw bytes before any processing ─────────
            # This prevents mid-loop IMAP state changes from breaking fetches.
            prefetched = []  # list of (uid_str, raw_bytes)
            for uid in uid_list:
                try:
                    uid_str = uid.decode() if isinstance(uid, bytes) else uid
                    fetch_status, msg_data = mail.uid('fetch', uid, '(RFC822)')

                    # Guard: Gmail sometimes returns [(None, None)] or empty list
                    if (fetch_status != 'OK'
                            or not msg_data
                            or msg_data[0] is None
                            or not isinstance(msg_data[0], tuple)
                            or msg_data[0][1] is None):
                        self._log(f"⚠️ Could not fetch UID {uid_str} — skipping")
                        continue

                    raw_bytes = msg_data[0][1]
                    prefetched.append((uid_str, raw_bytes))

                except Exception as fetch_err:
                    uid_str = uid.decode() if isinstance(uid, bytes) else str(uid)
                    self._log(f"⚠️ Fetch error for UID {uid_str}: {fetch_err} — skipping")
                    continue

            self._log(f"📥 Pre-fetched {len(prefetched)}/{len(uid_list)} emails — processing now")

            # ── Step 2: Process each email (IMAP state no longer matters) ─────
            for uid_str, raw_bytes in prefetched:
                try:
                    raw_size     = len(raw_bytes)
                    email_message = email.message_from_bytes(raw_bytes)
                    self.process_email(email_message, uid_str, raw_size, mail_conn=mail)
                except Exception as proc_err:
                    self._log(f"⚠️ Processing error for UID {uid_str}: {proc_err} — continuing")

                # Cooldown between emails to prevent CPU spike
                time.sleep(SECURITY_CONFIG["BATCH_COOLDOWN_SECONDS"])

            # ── Step 3: Single expunge — clears ALL \\Deleted flags at once ───
            # _apply_gmail_label() sets \\Deleted but never expunges.
            # We do it here, once, so sequence numbers only shift after the loop.
            try:
                mail.expunge()
                self._log("🗑️  Inbox expunged — labelled emails removed from INBOX")
            except Exception as exp_err:
                self._log(f"⚠️ Expunge failed (non-critical): {exp_err}")
